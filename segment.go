...
func (s *segment) draw(now time.Duration) {
	...
	imgWidth, imgHeight := len(img[0]), len(img)
	imgX, imgY := (s.column*imgWidth)/width, (y*imgHeight)/height
	hit := func() bool {
	...
	if hit() {
		termbox.SetCell((imgX*width)/imgWidth, (imgY*height)/imgHeight, s.rune(0), termbox.Attribute(s.colorShade(4)), termbox.ColorDefault)
		return
	}
